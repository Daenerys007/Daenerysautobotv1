...
async def auto_filter(bot: Client, update: Message):
    ...
    if len(results) == 0: # double check
        ...
        if len_result != 1:
            result[0].append(
                [
                    InlineKeyboardButton("Next j.j 27 ⏩", callback_data=f"navigate(0|next|{query})")
                ]
            )
        ...
        try:
            await bot.send_message(
                ...
                text=f"check what you want from @Fairytales27 Found {(len_results)} Results for your request from Daenerys: <code>{query}</code>",
